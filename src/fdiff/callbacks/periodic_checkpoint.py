...
class PeriodicCheckpoint(Callback):
    ...
    - Saves files named with a pattern like epoch{epoch:03d}.ckpt.
    - Epoch numbers are treated as 1-based (i.e., epoch 1 is the first epoch).
    """
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        # current_epoch is 0-based in Lightning; convert to 1-based for user-facing epochs
        epoch_num = int(trainer.current_epoch) + 1

        save = False
        if self.every_n_epochs and self.every_n_epochs > 0:
